...
export class DatabaseService implements OnModuleInit, OnModuleDestroy {
   ...
   * @param currentUserEmail Email of the currently connected user to exclude.
   * @returns {Promise<any[]>} Filtered array of users.
   */
  async getFilteredUsers(currentUserEmail?: string): Promise<any[]> {
    try {
      // Récupérer tous les utilisateurs
      const allUsers = await this.getAllUsers();

      // Filtrer les SuperAdmin et l'utilisateur actuel
      return allUsers.filter(user => {
        // Exclure les SuperAdmin
        if (user.role === 'SuperAdmin') {
          return false;
        }

        // Exclure l'utilisateur actuel si son email est fourni
  ...
  async deleteUser(email: string): Promise<boolean> {
    try {
      const query = `
        DELETE FROM \`${process.env.USER_BUCKET_NAME}\`
        WHERE email = $email
        RETURNING META().id;
      `;
